/* AetherLink slide figures.
   ...
   `window.FIGURES.forSlide(slide, ctx)` picks a figure kind from the slide
   type and kicker, then draws an inline SVG from the slide's own card titles,
   ...
   Kinds
     ...
     flow   how we use it, demo, example, theory: numbered steps ending at a human decision
...
(function(){
  ...
  /* ---- picker ------------------------------------------------------------ */
  function kindFor(s,type){const head=String(s.kicker||'').split('·')[0].trim().toLowerCase();const title=String(s.title||'').toLowerCase();
    ...
    if(/demo|example|theory|transfer/.test(head)||/demo|transfer|handoff|route from/.test(title))return'flow';
    if(type==='practice')return'cycle';
  ...
  const DRAW={hub,flow,cycle,gate,arc,close,pause,rows};
  function forSlide(s,ctx={}){const kind=ctx.kind||kindFor(s,ctx.type||'context');const draw=DRAW[kind]||hub;const svg=draw(s,ctx);svg.dataset.kind=kind;return svg;}
